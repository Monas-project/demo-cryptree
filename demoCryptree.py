...
class CryptTree:
    ...
    def grant_access(self, user):
        ...
        user.keys[self.name] = (
            self.DKf, self.BKf, self.SKf, self.FKf, self.CKf)
    ...
    def decrypt_nodes_recursively(self, node):
        ...
        if isinstance(node, Folder):
            print(f"Decrypting metadata for folder: {node.metadata}")
            print(f"node name :{node.name}")
            decrypted_metadata = node.decrypt_metadata(node)
            print(f"decrypted_metadata : {decrypted_metadata}")
            ...
            for file in node.files:
                print(f"file node name: {node.name}")
    ...
    def reencrypt_nodes(self, target_node):
        ...
        for node in reversed(nodes_to_reencrypt):
            ...
            # ノードがFolderの場合の処理
                ...
                node.SKf = new_SKf
                f = Fernet(new_SKf)
                encrypted_metadata = f.encrypt(
                    json.dumps(decrypted_metadata).encode())
                node.metadata = encrypted_metadata
                for child_folder in node.folders:
            ...
            if node.parent:
                node.BKf = new_BKf
                encrypted_parent_name = Fernet(
                    new_BKf).encrypt(node.parent.name.encode())
...
class File(CryptTree):
    def __init__(self, name, data, parent=None, location_uri=None, filename=None):
        ...
        self.metadata = self.encrypt_metadata(
            {"filename": self.filename, "creation_date": "some_date", "location": self.location_uri})
...
class Folder(CryptTree):
    ...
    def add_file(self, file):
        ...
        # BKfで親の名前を隠している
        f = Fernet(self.BKf)
        print(f"Encrypting parent name: {self.name}")
        encrypted_link = f.encrypt(file.name.encode())
        print(f"Encrypted parent name: {encrypted_link}")
        file.parent_link = encrypted_link
        print(f"Parent link: {file.parent_link}")
    ...
    def add_folder(self, folder):
        ...
        folder.metadata = self.encrypt_metadata()
    ...
    def decrypt_metadata(self, node):
        #print("Decrypting metadata for folder...")
        print(f"In decrypt関数 {node.metadata}")
...
def test_key_storage():
    ...
    # 4. 鍵を取得して、それが正しいか確認
    assert file.retrieve_my_keys() == (
        file.DKf, file.FKf), "File keys retrieval failed"
...
def test_decryption_process():
    ...
    file1 = File(name="file1", data="This is file data", filename="file1.txt")
    sub_folder.add_file(file1)

    # Step 2: Verify that the data and metadata are encrypted
    assert file1.data != "This is file data"
    assert isinstance(
        file1.metadata, bytes) or "filename" not in file1.metadata
    ...
    print(f"file1.data : {file1.data}")
    print(f"file1.metadata : {file1.metadata}")
    assert file1.data == "This is file data"
